...
type Screen = "dashboard" | "brief" | "structure" | "alternatives" | "editor";
...
const EN: Record<string, string> = {
  ...
  "上传主办方底图，并描述展位数量、通道、展区与销售要求。": "Upload the organizer’s venue plan and describe booth count, aisles, zones and sales requirements.", "上传底图": "Upload base plan", "描述展位规划需求": "Describe booth-planning requirements", "分析规划需求": "Analyze planning brief", "试试：": "Try:",
  ...
  "分区与动线": "Zoning & Circulation", "分区草案": "Zoning Draft", "双向分流": "Bidirectional Split", "顺时针参观": "Clockwise Visitor Flow", "逆时针参观": "Counter-clockwise Visitor Flow", "当前为规划草案": "Planning Draft", "作为可修改的规划偏好，不作为安全规则": "A revisable planning preference, not a safety rule", "仅在项目明确要求时采用": "Use only when explicitly requested", "适合多入口示例展馆，减少单向绕行": "Suitable for the multi-entrance sample and avoids forced one-way circulation", "展区面积配比": "Zone Area Allocation", "动线策略": "Circulation Strategy", "固定空间依据": "Fixed Spatial Evidence", "边界、8 个出入口和 6 个柱位来自示例底图，不由 AI 改写。": "Boundary, 8 entrances/exits and 6 columns come from the sample plan; AI cannot rewrite them.", "主通道结构": "Main Aisle Structure", "两条 6 m 主通道连接主要入口，横向通道保持连通。": "Two 6 m main aisles connect key entrances; the cross aisle remains connected.", "食品机械区 38%": "Food Machinery 38%", "包装技术区 34%": "Packaging Technology 34%", "配套服务区 18%": "Supporting Services 18%", "公共服务区 10%": "Public Services 10%", "动线方向是可解释偏好；消防疏散和正式合规仍需专业人员确认。": "Flow direction is an explainable preference; fire evacuation and formal compliance still require professional confirmation.", "返回调整约束": "Back to Constraints", "确认结构并生成 3 个展位方案 →": "Confirm Structure and Generate 3 Booth Plans →", "所选分区和动线结构将作为三个展位方案的共同输入。": "The selected zoning and circulation structure will be shared by all three booth plans.", "已确认分区与动线": "Zoning and Circulation Confirmed", "三个方案共享同一底图、分区草案和硬性规则，仅优化目标不同。": "All three plans share the same venue plan, zoning draft and hard rules; only their optimization priorities differ.", "展位视图": "Booth View", "分区视图": "Zone View", "动线视图": "Circulation View", "彩色分区视图（屏幕预览）": "Color-coded zone view (screen preview)", "动线叠加视图（屏幕预览）": "Circulation overlay (screen preview)", "确认分区动线": "Confirm Zoning and Circulation", "先确认粗粒度空间结构，再生成具体展位对象。": "Confirm the coarse spatial structure before generating booth objects.",
};
...
const zoningOptions = [
  { id: "bidirectional", title: "双向分流", note: "适合多入口示例展馆，减少单向绕行" },
  { id: "clockwise", title: "顺时针参观", note: "作为可修改的规划偏好，不作为安全规则" },
  { id: "counterclockwise", title: "逆时针参观", note: "仅在项目明确要求时采用" },
];
...
function ZoningPlan({ language, circulation, compact = false }: { language: Language; circulation: string; compact?: boolean }) {
  const t = (text: string) => translate(text, language);
  return <div className={`zoning-plan ${compact ? "compact" : ""}`} aria-label={t("分区草案")}>
    <div className="zone-block food"><b>{t("食品机械区")}</b><span>38%</span></div>
    <div className="zone-block pack"><b>{t("包装技术区")}</b><span>34%</span></div>
    <div className="zone-block service"><b>{t("配套服务区")}</b><span>18%</span></div>
    <div className="zone-block public"><b>{t("公共服务区")}</b><span>10%</span></div>
    <div className="route-line route-one"><i>→</i></div>
    <div className="route-line route-two"><i>{circulation === "clockwise" ? "↓" : circulation === "counterclockwise" ? "↑" : "↕"}</i></div>
    <div className="route-line route-cross"><i>{circulation === "clockwise" ? "→" : circulation === "counterclockwise" ? "←" : "↔"}</i></div>
    <em>{t(circulation === "clockwise" ? "顺时针参观" : circulation === "counterclockwise" ? "逆时针参观" : "双向分流")}</em>
  </div>;
}
...
export default function Home() {
  ...
  const [circulationMode, setCirculationMode] = useState("bidirectional");
  const [layoutView, setLayoutView] = useState<"booths" | "zones" | "circulation">("booths");
    ...
    return (
      <div className="dashboard-page home-page">
        ...
        <section className="ai-hero">
          <div className="hero-copy"><span>✦ {t("AI 展位规划助手")}</span><h2>{t("今天要规划哪个展馆？")}</h2><p>{t("上传主办方底图，并描述展位数量、通道、展区与销售要求。")}</p></div>
  ...
  if (screen === "brief" || screen === "structure" || screen === "alternatives") {
    return (
      <main className="flow-shell">
        ...
        <div className="flow-heading"><div><span className="kicker">{t("2026 中国国际食品展 · N3馆")}</span><h1>{t(screen === "brief" ? "确认 AI 理解的规划条件" : screen === "structure" ? "分区与动线" : "选择一个候选方案")}</h1><p>{t(screen === "brief" ? "生成前先核对硬性规则、规划偏好和信息来源。" : screen === "structure" ? "先确认粗粒度空间结构，再生成具体展位对象。" : "三个方案共享同一底图、分区草案和硬性规则，仅优化目标不同。")}</p></div><FlowSteps active={screen === "brief" ? 2 : screen === "structure" ? 3 : 4} language={language} /></div>
        {screen === "brief" ? <section className="brief-layout">
          ...
          <div className="constraint-panel"><div className="constraint-head"><div><span>✦ {t("AI 已整理")}</span><h2>{t("6 条候选约束")}</h2><p>{t("“必须满足”将由规则检查独立验证，AI 不能覆盖检查结果。")}</p></div><button onClick={addConstraint}>＋ {t("添加条件")}</button></div><div className="constraint-list">{constraints.map((item) => <article className={!item.active ? "disabled" : ""} key={item.id}><button className={`check ${item.active ? "on" : ""}`} onClick={() => toggleConstraint(item.id)}>{item.active ? "✓" : ""}</button><div className="constraint-copy"><div><h3>{t(item.title)}</h3><span className={item.severity === "必须满足" ? "hard" : "soft"}>{t(item.severity)}</span></div><input value={t(item.value)} onChange={(event) => setConstraints((items) => items.map((current) => current.id === item.id ? { ...current, value: event.target.value } : current))} /><small>{t("来源：")}{t(item.source)}</small></div><button className="more" disabled title={t("暂不支持")}>•••</button></article>)}</div><div className="flow-actions"><button className="secondary" onClick={() => setScreen("dashboard")}>{t("返回修改需求")}</button><div><span>{t(`${constraints.filter((item) => item.active).length} 条条件将进入规划核心`)}</span><button className="primary" onClick={() => setScreen("structure")}>{t("确认分区动线")} →</button></div></div></div>
        </section> : screen === "structure" ? <section className="structure-section">
          <div className="structure-preview-card"><header><div><span>03 · {t("分区与动线")}</span><h2>{t("分区草案")}</h2></div><em>{t("当前为规划草案")}</em></header><ZoningPlan language={language} circulation={circulationMode} /><div className="structure-facts"><article><span>✓</span><div><b>{t("固定空间依据")}</b><p>{t("边界、8 个出入口和 6 个柱位来自示例底图，不由 AI 改写。")}</p></div></article><article><span>↔</span><div><b>{t("主通道结构")}</b><p>{t("两条 6 m 主通道连接主要入口，横向通道保持连通。")}</p></div></article></div></div>
          <aside className="structure-controls"><section><h2>{t("展区面积配比")}</h2><div className="zone-allocation"><p><i className="food" /><b>{t("食品机械区 38%")}</b><span>{language === "en" ? "Near north entrance" : "靠近北入口"}</span></p><p><i className="pack" /><b>{t("包装技术区 34%")}</b><span>{language === "en" ? "Central hall" : "展馆中部"}</span></p><p><i className="service" /><b>{t("配套服务区 18%")}</b><span>{language === "en" ? "East side" : "东侧区域"}</span></p><p><i className="public" /><b>{t("公共服务区 10%")}</b><span>{language === "en" ? "Near fixed facilities" : "邻近固定设施"}</span></p></div></section><section><h2>{t("动线策略")}</h2><div className="circulation-options">{zoningOptions.map((option) => <button className={circulationMode === option.id ? "selected" : ""} key={option.id} onClick={() => setCirculationMode(option.id)}><span>{circulationMode === option.id ? "✓" : ""}</span><div><b>{t(option.title)}</b><small>{t(option.note)}</small></div></button>)}</div></section><div className="structure-boundary"><span>!</span><p><b>{t("当前为规划草案")}</b>{t("动线方向是可解释偏好；消防疏散和正式合规仍需专业人员确认。")}</p></div><div className="flow-actions"><button className="secondary" onClick={() => setScreen("brief")}>← {t("返回调整约束")}</button><button className="primary" onClick={() => setScreen("alternatives")}>{t("确认结构并生成 3 个展位方案 →")}</button></div></aside>
        </section> : <section className="alternative-section"><div className="alternative-notice"><span>✓</span><div><b>{t("已确认分区与动线")}</b><small>{t("所选分区和动线结构将作为三个展位方案的共同输入。")}</small></div></div><div className="variant-grid">{variants.map((variant) => <button className={`variant-card ${selectedVariant === variant.id ? "selected" : ""}`} key={variant.id} onClick={() => setSelectedVariant(variant.id)}><div className="variant-top"><span>{t("方案")} {variant.id}</span>{variant.id === "B" && <em>{t("推荐")}</em>}<i>{selectedVariant === variant.id ? "✓" : ""}</i></div><MiniPlan tone={variant.tone} dense={variant.id === "C"} /><h2>{t(variant.name)}</h2><p>{t(variant.note)}</p><dl><div><dt>{t("展位数量")}</dt><dd>{variant.booths} {language === "en" ? "" : "个"}</dd></div><div><dt>{t("可售面积")}</dt><dd>{variant.area.toLocaleString()} ㎡</dd></div><div><dt>{t("未解决规则")}</dt><dd className={variant.issues ? "warning" : "success"}>{variant.issues} {language === "en" ? "" : "条"}</dd></div><div><dt>{t("已售展位变更")}</dt><dd>{variant.sold} {language === "en" ? "" : "个"}</dd></div></dl><span className="variant-link">{t("查看方案细节 →")}</span></button>)}</div><div className="tradeoff"><div><span>✦</span><p><b>{t("为什么推荐方案 B？")}</b> {language === "en" ? "It has no unresolved hard-rule issues, balances entrance circulation with sellable area, and does not move any sold booth." : "它没有未解决硬性规则，在入口分流和可售面积之间更均衡，并且不移动任何已售展位。"}</p></div></div><div className="flow-actions wide"><button className="secondary" onClick={() => setScreen("structure")}>← {t("返回调整约束")}</button><div><span>{language === "en" ? `Selected Plan ${selectedVariant}` : `已选择方案 ${selectedVariant}`}</span><button className="primary" onClick={() => setScreen("editor")}>{t("进入专业工作台 →")}</button></div></div></section>}
      </main>
    );
  }
  ...
  return (
    <main className="editor-shell">
      ...
      <section className="editor-progress"><FlowSteps active={5} compact language={language} /><div className="plan-tabs"><button className="version-button" onClick={() => setShowHistory(true)}>↺ {currentVersionId} · {t("版本记录")}</button><span>{t("当前方案")}</span>{variants.map((variant) => <button key={variant.id} className={selectedVariant === variant.id ? "active" : ""} onClick={() => setSelectedVariant(variant.id)}>{t("方案")} {variant.id}</button>)}</div></section>
      <section className={`workspace ${inspectorVisible ? "" : "inspector-hidden"}`}>
        ...
        <section className="canvas-panel">
          <div className="canvas-toolbar"><div className="view-switch"><button className={layoutView === "booths" ? "active" : ""} onClick={() => setLayoutView("booths")}>{t("展位视图")}</button><button className={layoutView === "zones" ? "active" : ""} onClick={() => setLayoutView("zones")}>{t("分区视图")}</button><button className={layoutView === "circulation" ? "active" : ""} onClick={() => setLayoutView("circulation")}>{t("动线视图")}</button><span /></div><div><button className={canvasTool === "select" ? "active" : ""} onClick={() => setCanvasTool("select")}>↖ {t("选择")}</button><button className={canvasTool === "pan" ? "active" : ""} onClick={() => setCanvasTool("pan")}>✋ {t("平移")}</button><button className={canvasTool === "box" ? "active" : ""} onClick={() => setCanvasTool("box")}>□ {t("框选")}</button><button onClick={() => versions.length > 0 && restoreVersion(versions[0])} title={language === "en" ? "Restore initial version" : "恢复初始版本"}>↶</button><button disabled title={language === "en" ? "No redo step" : "暂无重做步骤"}>↷</button></div><div><button onClick={() => setCanvasZoom((value) => Math.max(40, value - 10))}>−</button><b>{canvasZoom}%</b><button onClick={() => setCanvasZoom((value) => Math.min(140, value + 10))}>＋</button><button onClick={() => setCanvasZoom(78)}>{t("适应画布")}</button><button className={inspectorVisible ? "active" : ""} onClick={() => setInspectorVisible((value) => !value)}>☷ {t("属性面板")}</button></div></div>
          <div className={`floorplan-wrap ${generated ? "generated" : ""}`}>
            ...
            <div className="floorplan professional-cad" style={{ transform: `scale(${canvasZoom / 78})`, transformOrigin: "center center" }}>
              ...
              {layoutView === "zones" && <div className="editor-zone-overlay"><span className="food">{t("食品机械区")} · 38%</span><span className="pack">{t("包装技术区")} · 34%</span><span className="service">{t("配套服务区")} · 18%</span><span className="public">{t("公共服务区")} · 10%</span></div>}
              {layoutView === "circulation" && <div className="editor-circulation-overlay"><i className="vertical-one">↓</i><i className="vertical-two">↑</i><i className="horizontal-route">↔</i><em>{t(circulationMode === "clockwise" ? "顺时针参观" : circulationMode === "counterclockwise" ? "逆时针参观" : "双向分流")}</em></div>}
...
function FlowSteps({ active, compact = false, language }: { active: number; compact?: boolean; language: Language }) {
  const steps = ["导入底图", "确认要求", "分区与动线", "候选方案", "规划修改", "专业交付"];
...
function ExportDialog({ language, booths, version, ruleResults, onClose }: { language: Language; booths: Booth[]; version: string; ruleResults: RuleResult[]; onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="export-dialog" role="dialog" aria-modal="true" aria-labelledby="export-title"><button className="modal-close" onClick={onClose}>×</button><header><span>↧</span><div><h2 id="export-title">{t("导出交付预览")}</h2><p>{t("导出内容来自当前画布与实时规则结果。")}</p></div></header><div className="export-preview"><ZoningPlan language={language} circulation="bidirectional" compact /><div><b>N3 HALL · {version}</b><p>{booths.length} {language === "en" ? "booth objects" : "个展位对象"}</p><p>{ruleResults.filter((rule) => rule.status === "通过").length} {t("项通过")} · {ruleResults.filter((rule) => rule.status === "提醒").length} {t("项提醒")} · {ruleResults.filter((rule) => rule.status === "阻断").length} {t("项阻断")}</p></div></div><div className="export-list"><p><span>✓</span>{t("展位对象与编号")}</p><p><span>✓</span>{t("规则检查报告")}</p><p><span>✓</span>{t("版本与项目信息")}</p><p><span>✓</span>{t("彩色分区视图（屏幕预览）")}</p><p><span>✓</span>{t("动线叠加视图（屏幕预览）")}</p></div><footer><button className="secondary" onClick={exportCsv}>{t("下载 CSV 数据")}</button><button className="primary" onClick={exportDxf}>{t("下载 DXF 演示文件")}</button></footer></section></div>;
}
